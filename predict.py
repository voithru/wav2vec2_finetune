...
def predict_one():
    ...
    for file_path in tqdm(file_list):
        ...
        inputs = processor(audio, sampling_rate=standard_sample_rate, return_tensors="pt")
        ...
        file = open(args['output_dir'] + f"/{file_path.stem}.json", "w", encoding="utf-8")
...
def predict_all():
    ...
    for checkpoint in tqdm(checkpoints):
        ...
        for (file_name, audio) in audios:
            ...
            file = open(args['output_dir'] + f"/{checkpoint.stem}/{file_name}.json", "w", encoding="utf-8")
            line = json.dumps({"text": transcription}, ensure_ascii=False)
            file.write(line)
            file.close()
